package io.anuke.someplanets.noise;
...
public class CaveGen extends MapGenBaseMeta {
	@Override
	...
	protected void recursiveGenerate(World par1World, int xChunkCoord, int zChunkCoord, int origXChunkCoord,
		...
		if (this.rand.nextInt(5) == 0) {
			...
			this.generateLargeCaveNode(this.rand.nextLong(), origXChunkCoord, origZChunkCoord, arrayOfIDs, arrayOfMeta,
					xPos, yPos, zPos);
		}
	}
	...
	protected void generateLargeCaveNode(long par1, int origXChunkCoord, int origZChunkCoord, Block[] arrayOfIDs,
		...
		this.generateCaveNode(par1, origXChunkCoord, origZChunkCoord, arrayOfIDs, arrayOfMeta, xPos, yPos, zPos,
				1.0F + this.rand.nextFloat() * 5.0F, 8.0F, 8.0F, -1, -1, 2D);
	}
	...
	protected void generateCaveNode(long par1, int origXChunkCoord, int origZChunkCoord, Block[] arrayOfIDs,
		...
		for (final boolean var28 = var25.nextInt(6) == 0; par15 < par16; ++par15) {
			final double caveWidth = 10+var25.nextFloat()*4;
			final double caveHeight = caveWidth * (heightMultiplier+var25.nextFloat()/3f);
			...
			if (var54 || var25.nextInt(4) != 0) {
				...
				if (xPos >= var19 - 16.0D - caveWidth * 2.0D && zPos >= var21 - 16.0D - caveWidth * 2.0D
					...
					if (caveMinY < 26+var25.nextInt(5)) {
						caveMinY = 26+var25.nextInt(5);
					}
									...
									if (var59 * var59 + var51 * var51 + var46 * var46 < 1.0D) {
										if (var51 > -0.7D) {
											...
											if (Blocks.air == arrayOfIDs[coords]) {
												if (arrayOfIDs[coordsAbove] != null && arrayOfIDs[coordsAbove].getMaterial().isSolid()) {
													if (this.rand.nextInt(40) == 0) {
														int modifier = 0;
														int len = rand.nextInt(19) + 1;
														int i = 0;
														while (i++ < len) {
															arrayOfIDs[coordsBelow + 1] = Blocks.stone;
															arrayOfMeta[coordsBelow
																	+ 1] = (byte) (Math.abs(modifier) % 3);
															modifier--;
															coordsBelow = (var42 * 16 + var45) * 256 + var50 - 1
																	+ modifier;
														}
													} else if (this.rand.nextInt(200) == 0) {
														arrayOfIDs[coords] = Blocks.glowstone;
